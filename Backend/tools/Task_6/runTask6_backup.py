from tools.Task_6.scale_ratio import convert_dxf_to_png_scale_ratio
import os
import sys
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
import glob
import torch
from tools.Task_6.Unet import Unet
...
def runTask6(session_dir, single=True):
    ...
    dxf_files = files.get("task6", [])
 
    if not dxf_files:
        return "Không tìm thấy file DXF cho task6"
    dxf_file = dxf_files[0]  

    output_image_path = os.path.join(output_folder, "Task_6.png")

    ratio = convert_dxf_to_png_scale_ratio(dxf_file, output_folder, layers_rg_qh, layers_to_check, dpi=300, bg='#FFFFFF')
    pixel_length, pixel_width = 100/ ratio, 7/ ratio

    def process_drawing(image_path,unet_checkpoint, avoid_model_path, dpi=300,
        ...
        #plt.title("Final segments with avoid boxes")
        #plt.axis("off")
        #plt.show()
        # ================= Predict mask
        ...
        #plt.figure(figsize=(10, 10))
        #plt.imshow(cv2.cvtColor(mask_binary, cv2.COLOR_BGR2RGB))
        #plt.axis("off")
        #plt.show()
    ...
    try:
        final_segments, valid_boxes, invalid_boxes, result_img = process_drawing(
            output_image_path,
            unet_checkpoint=r"D:\LAB\project_bcons\bcons_app\Backend\tools\Task_6\my_checkpoint.pth.tar",
    ...
    for idx, ((x1, y1, x2, y2), length, width) in enumerate(final_segments):
        result_str += f"Đoạn {idx+1}: ({x1},{y1})-({x2},{y2}) | Chiều dài: {(length*ratio):.1f}px | Chiều rộng: {(width*ratio):.1f}px<br>"

    result_str += "Đoạn tránh xe hợp lệ:<br>"
    for box in valid_boxes:
        result_str += f"Đoạn tránh xe hợp lệ {box['id']} | Chiều dài: {(box['length']*ratio):.1f} | Chiều rộng: {(box['width']*ratio):.1f}<br>"

    result_str += "Đoạn tránh xe không hợp lệ:<br>"
    for box in invalid_boxes:
        result_str += f"Đoạn tránh xe không hợp lệ {box['id']} | Chiều dài: {(box['length']*ratio):.1f} | Chiều rộng: {(box['width']*ratio):.1f}<br>"
